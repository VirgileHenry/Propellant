...
impl Camera {
    ...
    pub fn main_orthographic(screen_height: f32, screen_width: f32, z_near: f32, z_far: f32) -> Camera {
        ...
        let half_screen_height = screen_height * 0.5;
        let proj = glam::Mat4::orthographic_rh(-half_screen_width, half_screen_width, -half_screen_height, half_screen_height, z_near, z_far);
        Camera {
            properties: CameraTypeProperty::Orthographic(screen_height, screen_width, z_near, z_far),
    ...
    pub fn resize(&mut self, new_screen_height: f32, new_screen_width: f32) {
        match &mut self.properties {
            ...
            CameraTypeProperty::Orthographic(height, width, z_near, z_far) => {
                *height = new_screen_height;
                *width = new_screen_width;
                let half_screen_width = new_screen_width * 0.5;
                let half_screen_height = new_screen_height * 0.5;
                self.projection_matrix = glam::Mat4::orthographic_rh_gl(-half_screen_width, half_screen_width, -half_screen_height, half_screen_height, *z_near, *z_far);
            },
        }
    }
}